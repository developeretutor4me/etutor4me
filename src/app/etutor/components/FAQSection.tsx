  ...
  return (
    <div className=" w-full h-fit flex flex-col custom-2xl:flex-row">
      <div className='  w-full h-full'>
        <h2 className="text-lg sm:text-2xl custom-2xl:text-[45px] custom-2xl:leading-[2.25rem] font-bold text-[#685AAD] mb-0 ml-4">Frequently Asked Questions</h2>
        <ul className="mt-4 px-2 custom-2xl:px-9  custom-2xl:mt-10 ">
          {faqs.map((faq, index) => (
            <li key={index} className="py-4 custom-2xl:py-11 text-xs sm:text-lg custom-2xl:text-[28.15px] leading-[2rem]  border-b text-[#534988]  border-[#ada7cfc4] last:border-b-0  ">
              <button
              ...
              >
                ...
                <Image loading="lazy" src={downarrow} alt="" className={` sm:h-4 custom-2xl:h-7  h-2 w-2 sm:w-4 custom-2xl:w-7 text-black font-thin transition-transform ${openIndex === index ? 'transform rotate-180' : ''}`} />
