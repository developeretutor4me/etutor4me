...
const SessionCalendarComponent = ({
  ...
  const filteredSessions = useMemo(() => {
    ...
    return requests.filter((session:any) => {
              ...
              {activeSessionTab === "confirmed" && (
                <>
                  <div className="">
                    ...
                    {requests.filter((request:any) => (request.status === "accepted") )
                      .length !== 0 ? (
                      <div className="flex flex-col gap-2 custom-xl:gap-3">
                        {requests
                          .filter((request:any) => request.status === "accepted" && (!request.meetingCompleted))
                          .map((request) => {
                            const isExpanded =
                              expandedRequestId === request._id;
                            return (
