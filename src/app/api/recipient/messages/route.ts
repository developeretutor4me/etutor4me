...
export async function GET(req: NextRequest) {
  try {
      ...
      conversations.map(async (conversation: any) => {
        ...
        try {
          ...
          if (user.role === "student") {
            senderDetails = await Student.findOne({ user: user._id }).populate(
              "user"
            );
          } else if (user.role === "parent") {
            senderDetails = await Parent.findOne({ user: user._id }).populate(
              "user"
            );
    ...
    const uniqueSenders = Array.from(
      ...
      senders.find((sender) => sender && sender._id.toString() === id)
    );
